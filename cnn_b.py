...
x_train = np.reshape(inp[:, :-1],
                     (inp.shape[0], 32, 32, 3))
y_train = keras.utils.to_categorical(inp[:, -1:], 10)
...
# accuracy increase by subtracting pixel mean
...
checkpoint = ModelCheckpoint(filepath="bestmodel_b.hdf5",
                             monitor='val_acc',
                             verbose=1,
                             save_best_only=True)
...
callbacks = [checkpoint, lr_reducer, lr_scheduler, earlystopper]
...
model.load_weights("bestweights_b.hdf5")
